...
usage:
[optional: Copy your MapEdit.dat/cfg to a directory along with ConfigConverter.exe.] Run the converter, it's straightforward. (no, i don't know how to make a virus, trojan, or whatever they are) The output should be a logfile (deletable) and MapConfig.yml - copy/move the latter to your MapView 2 "settings" folder and try running MapView 2.

The definition of MapConfig.yml is written at the top of the file itself, in case you want to review/edit it by hand. Note: YAML is editable as plaintext, but never use a tab character in YAML.
