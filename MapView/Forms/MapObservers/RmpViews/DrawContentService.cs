...
namespace MapView.Forms.MapObservers.RmpViews
{
    public class DrawContentService
    {
        ...
        public void DrawContent(Graphics g, SolidPenBrush color, int x, int y, TileBase content)
        {
            ...
            var topCorner = new Point(x, y + TO_WALL_MARGIN);
            var bottomCorner = new Point(x, y + (HHeight * 2) - TO_WALL_MARGIN);
            var leftCorner = new Point(x - HWidth + (TO_WALL_MARGIN * 2), y + HHeight);
            var rightCorner = new Point(x + HWidth - (TO_WALL_MARGIN * 2), y + HHeight);
            ...
            switch (contentType)
            {
                ...
                case ContentTypes.Ground:
                    SetGroundPath(x, y);
                    g.FillPath(color.LightBrush, _content);
                    break;
                case ContentTypes.NorthFence:
                    g.DrawLine(color.LightPen, topCorner, rightCorner);
                    break;
                ...
                case ContentTypes.WestFence:
                    g.DrawLine(color.LightPen, topCorner, leftCorner);
                    break;
